...
describe('retirerCourse', () => {
  it('décrémente les compteurs du jour', async () => {
    await fin(600, 10);
    await fin(300, 5);

    await store().retirerCourse({
      tempsEcoule: 300,
      revenu: 5,
      date: '2026-09-08',
    });
    ...
    expect(stats().nbCourses).toBe(1);
    expect(stats().tempsTotal).toBe(600);
    expect(stats().revenuTotal).toBe(10);
    expect(storage.sauvegarder).toHaveBeenCalledWith(
      '@vtc_stats_jour',
      expect.objectContaining({ nbCourses: 1 }),
    );
  });

  it('ne touche à rien si la date ne correspond plus (minuit passé)', async () => {
    await fin(600, 10);

    await store().retirerCourse({
      tempsEcoule: 600,
      revenu: 10,
      date: '2026-09-07', // course comptée la veille
    });
  ...
  it('ne descend jamais sous zéro', async () => {
    ...
    expect(stats().nbCourses).toBe(0);
    expect(stats().tempsTotal).toBe(0);
    expect(stats().revenuTotal).toBe(0);
  });
});
